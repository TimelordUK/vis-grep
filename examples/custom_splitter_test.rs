// REFERENCE EXAMPLE: Custom Splitter Implementation
// 
// This demonstrates a working horizontal/vertical splitter for egui.
// 
// WHY THIS EXISTS:
// egui 0.29's built-in TopBottomPanel::resizable() is broken and won't
// allow dragging the splitter. This custom implementation uses direct
// drag handling with Sense::click_and_drag() and works correctly.
//
// Based on: https://gist.github.com/mkalte666/f9a982be0ac0276080d3434ab9ea4655
// Modified for egui 0.29 API (child_ui now requires 3 args)
//
// USAGE:
// Run with: cargo run --example custom_splitter_test --release
// Try dragging the horizontal line to verify the splitter works!
...
/// The internal data used by a splitter
#[derive(Debug, Clone)]
struct SplitterData {
    axis: SplitterAxis,
    pos: f32,
...
/// Splits a ui in half, using a draggable separator
pub struct Splitter {
    id: Id,
    data: SplitterData,
}
...
fn main() -> eframe::Result<()> {
    let native_options = eframe::NativeOptions {
        viewport: egui::ViewportBuilder::default()
            ...
            .with_title("Custom Splitter Test"),
        ..Default::default()
    };

    eframe::run_simple_native("custom_splitter_test", native_options, move |ctx, _frame| {
        egui::CentralPanel::default().show(ctx, |ui| {
            ui.heading("Custom Splitter Test");
            ui.separator();
            
            // Vertical split (horizontal divider)
            Splitter::new("vertical_split", SplitterAxis::Vertical)
                .min_size(100.0)
                .default_pos(0.3)
                .show(ui, |ui_top, ui_bottom| {
                    ui_top.heading("TOP PANEL");
                    ui_top.label("This should be the top 30%");
                    ui_top.label("Drag the horizontal line below to resize");
                    
                    ui_bottom.heading("BOTTOM PANEL");
                    ui_bottom.label("This should be the bottom 70%");
                    ui_bottom.label("If you can drag the splitter, the custom implementation works!");
                });
        });
    })
}
